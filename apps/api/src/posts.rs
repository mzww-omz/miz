...
pub struct PostResponse {
    id: PostId,
    author_id: UserId,
    reply_to_post_id: Option<PostId>,
...
pub async fn create_post(
    ...
    .map_err(internal_error)?;
    sqlx::query(
        "INSERT INTO posts (id, author_id, content, effective_visibility) VALUES ($1, $2, $3, $4)",
    )
...
async fn load_post(
    ...
    let row: PostRow = sqlx::query_as(
        "SELECT p.id, p.author_id, p.reply_to_post_id, p.content, p.effective_visibility, p.state, p.version, \
         ...
         EXISTS (SELECT 1 FROM follow_relationships f WHERE f.follower_id = $2 AND f.followee_id = p.author_id AND f.status = 'accepted') \
         FROM posts p WHERE p.id = $1 AND p.state IN ('published', 'tombstone')",
    )
    .bind(post_id.to_bytes().to_vec())
    .bind(principal.user_id.to_bytes().to_vec())
    ...
    .map_err(internal_error)?
    .ok_or_else(not_found)?;
    let author_id = user_id(&row.1)?;
    let can_read =
        row.4 == "public" || authorize(principal, Action::ReadFollowersOnly, author_id, row.10);
    if !can_read {
        return Err(not_found());
    }
    Ok(PostResponse {
